...
\subsection{DVR and Dedekind Domain}

    \begin{definition}\label{def:valuation}
        A \emph{valuation} on a field \(K\) is a function \(v: K^\times \to \Gamma\), where \(\Gamma\) is a totally ordered abelian group, such that for all \(x, y \in K^\times\):
        \begin{enumerate}[label=(\roman*)]
            \item \(v(xy) = v(x) + v(y)\).
            \item \(v(x+y) \geq \min(v(x), v(y))\) if \(x+y \neq 0\).
        \end{enumerate}
        We extend \(v\) to \(K\) by setting \(v(0) = +\infty\), where \(+\infty\) is an element greater than all elements of \(\Gamma\).

        If \(\Gamma\) is discrete with respect to the order topology, we say that \(v\) is a \emph{discrete valuation}.
    \end{definition}

    \begin{example}\label{eg:valuation}
        \begin{enumerate}
            \item Let \(\KK = \bbQ\) and \(p\) be a prime number.
                Let \(v: \bbQ^\times \to \bbZ\) be defined by \(v(a/b) = n\) if \(a/b = p^n (c/d)\) with \(c,d\) coprime to \(p\).
                Then \(v\) is a discrete valuation on \(\bbQ\).
            
            \item Let \(\KK = \kk(T)\) be the field of rational functions over a field \(\kk\).
                For \(f = x^n g \in \kk(t)\) with \(g(0) \in \kk^\times\), let \(v(f) = n\).
            
            \item Let \(\KK = \bbC_p\) the \(p\)-adic complex numbers.
                For \(x \in \bbC_p^\times\), let \(v(x) = -\log_p |x|_p\).
                Then \(v\) is a valuation on \(\bbC_p\) which is not discrete.
        \end{enumerate}
    \end{example}

    \begin{definition}[Discrete Valuation Ring]\label{def:DVR}
        Let \(V\) be a domain with field of fractions \(\KK\).
        We say that \(V\) is a \emph{discrete valuation ring (DVR)} if there exists a discrete valuation \(v\) on \(\KK\) such that \(V = \{x \in \KK \mid v(x) \geq 0\}\).
    \end{definition}
...
\subsection{Cohen-Macaulay rings}
    ...
    \begin{proposition}\label{thm: unmixedness theorem for Cohen-Macaulay schemes}
        Let $X$ be a locally noetherian scheme.
    ...
    \begin{theorem}\label{thm:Cohen-Macaulay_and_local_cohomology}
        Let \(A\) be a noetherian ring of dimension \(d\). The following are equivalent:
        \Yang{To be completed.}
    \end{theorem}
    \begin{proof}
        \Yang{To be completed.}
...
\subsection{Regular rings}
    ...
    \begin{remark}
        Indeed we can show a stronger result: a noetherian regular local ring is a UFD; see \Yang{ref}.
    \end{remark}
